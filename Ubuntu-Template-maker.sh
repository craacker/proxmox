...
if [[ ! " ${VALID_VERSIONS[*]} " =~ " ${UBUNTU_VERSION} " ]]; then
  echo "❌ Error: Invalid Ubuntu version. Please enter one of: ${VALID_VERSIONS[*]}"
  exit 1
fi
...
echo "➡️ Using VMID: $VMID"
echo "⬇️ Downloading Ubuntu ${UBUNTU_VERSION} cloud image..."
wget -O $IMG_FILE $DOWNLOAD_URL

if [[ $? -ne 0 ]]; then
  echo "❌ Failed to download the image. Please check the version or your internet connection."
  exit 1
fi
...
echo "✅ Template created: $VMNAME (VMID: $VMID)"
